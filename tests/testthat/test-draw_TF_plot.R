test_that("is input dataset correct?", {
  ...
  expect_error(draw_TF_plot(1:10), 'Input dataset must be data.frame')
})
test_that('is promoter length more than zero?', {
  test_df <- data.frame(
    TF = c("TF1", "TF2", "TF3"),
    rel_beg = c(-1000, -700, -400),
    rel_end = c(-800, -450, -300),
  ...
  expect_error(draw_TF_plot(test_df, -1500), 'Promoter length must be more than zero')
})
